...
Future<dynamic> getShopByName(String name) async {
  late ShopModel shop ;
    try{
       String url = "http://192.168.1.13:5000/shops/getShopByName?name=${name}";
...
Future<ShopModel> getShopById(String id) async {
  late ShopModel shop ;
    try{
       String url = "http://192.168.1.13:5000/shops/getShop/${id}";
       final http.Response resp= await http.get(Uri.parse(url),
       headers: <String, String> {'Content-Type': 'application/json; charset=UTF-8'},
       );
       if(resp.statusCode == 200){
...
Future<List<dynamic>> CalculateDistance ( double latitude, double longitude) async {
  String url = "http://192.168.1.13:5000/shops/CalculateDistance";
  try{
...
Future<List<dynamic>> fetchShopProducts(String shopId) async {
  final url = 'http://192.168.1.13:5000/shops/getShopProducts/$shopId';
